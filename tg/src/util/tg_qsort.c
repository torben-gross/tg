...
static i32 tgi_partition(u32 element_size, u8* p_elements, i32 low, i32 high, tg_qsort_compare_fn* p_compare_fn)
{
...
static void tgi_impl(u32 element_size, u8* p_elements, i32 low, i32 high, tg_qsort_compare_fn* p_compare_fn)
{
    if (low < high)
    {
        const i32 pi = tgi_partition(element_size, p_elements, low, high, p_compare_fn);
        tgi_impl(element_size, p_elements, low, pi - 1, p_compare_fn);
        tgi_impl(element_size, p_elements, pi + 1, high, p_compare_fn);
    }
}
...
void tg_qsort_impl(u32 element_size, u32 element_count, void* p_elements, tg_qsort_compare_fn* p_compare_fn)
{
    ...
    tgi_impl(element_size, (u8*)p_elements, 0, element_count - 1, p_compare_fn);
}
