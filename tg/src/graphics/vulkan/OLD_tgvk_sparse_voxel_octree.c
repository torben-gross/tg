...
void tg_voxelizer_create(tg_voxelizer* p_voxelizer)
{
    ...
    graphics_pipeline_create_info.p_vertex_shader = &tg_vertex_shader_create("shaders/voxelize.vert")->shader;
    graphics_pipeline_create_info.p_fragment_shader = &tg_fragment_shader_create("shaders/voxelize.frag")->shader;
    ...
    p_voxelizer->pipeline = tgvk_pipeline_create_graphics(&graphics_pipeline_create_info);
    ...
    p_voxelizer->p_image_3ds[2] = TGVK_IMAGE_3D_CREATE(TGVK_IMAGE_TYPE_STORAGE, TG_SVO_DIMS, TG_SVO_DIMS, TG_SVO_DIMS, (VkFormat)TG_COLOR_IMAGE_FORMAT_R32_UINT, TG_NULL);
    p_voxelizer->p_image_3ds[3] = TGVK_IMAGE_3D_CREATE(TGVK_IMAGE_TYPE_STORAGE, TG_SVO_DIMS, TG_SVO_DIMS, TG_SVO_DIMS, (VkFormat)TG_COLOR_IMAGE_FORMAT_R32_UINT, TG_NULL);
    p_voxelizer->p_image_3ds[4] = TGVK_IMAGE_3D_CREATE(TGVK_IMAGE_TYPE_STORAGE, TG_SVO_DIMS, TG_SVO_DIMS, TG_SVO_DIMS, (VkFormat)TG_COLOR_IMAGE_FORMAT_R32_UINT, TG_NULL);
    p_voxelizer->p_image_3ds[5] = TGVK_IMAGE_3D_CREATE(TGVK_IMAGE_TYPE_STORAGE, TG_SVO_DIMS, TG_SVO_DIMS, TG_SVO_DIMS, (VkFormat)TG_COLOR_IMAGE_FORMAT_R32_UINT, TG_NULL);
    p_voxelizer->p_image_3ds[6] = TGVK_IMAGE_3D_CREATE(TGVK_IMAGE_TYPE_STORAGE, TG_SVO_DIMS, TG_SVO_DIMS, TG_SVO_DIMS, (VkFormat)TG_COLOR_IMAGE_FORMAT_R32_UINT, TG_NULL);
    p_voxelizer->p_image_3ds[7] = TGVK_IMAGE_3D_CREATE(TGVK_IMAGE_TYPE_STORAGE, TG_SVO_DIMS, TG_SVO_DIMS, TG_SVO_DIMS, (VkFormat)TG_COLOR_IMAGE_FORMAT_R32_UINT, TG_NULL);
    ...
    {
        tgvk_cmd_transition_image_3d_layout(&p_voxelizer->command_buffer, &p_voxelizer->p_image_3ds[i], TGVK_LAYOUT_UNDEFINED, TGVK_LAYOUT_SHADER_WRITE_F);
    }
    tgvk_command_buffer_end_and_submit(&p_voxelizer->command_buffer);
    for (u32 i = 0; i < TG_SVO_ATTACHMENTS; i++)
    {
        const VkDeviceSize format_size = tg_color_image_format_size((tg_color_image_format)p_voxelizer->p_image_3ds[i].format);
        p_voxelizer->p_voxel_buffers[i] = TGVK_BUFFER_CREATE(TG_SVO_DIMS3 * format_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT, TGVK_MEMORY_HOST);
    }
}
...
void tg_voxelizer_begin(tg_voxelizer* p_voxelizer)
{
    ...
    for (u32 i = 0; i < TG_SVO_ATTACHMENTS; i++)
    {
        tgvk_cmd_transition_image_3d_layout(&p_voxelizer->command_buffer, &p_voxelizer->p_image_3ds[i], TGVK_LAYOUT_SHADER_WRITE_F, TGVK_LAYOUT_TRANSFER_WRITE);
        tgvk_cmd_clear_image_3d(&p_voxelizer->command_buffer, &p_voxelizer->p_image_3ds[i]);
        tgvk_cmd_transition_image_3d_layout(&p_voxelizer->command_buffer, &p_voxelizer->p_image_3ds[i], TGVK_LAYOUT_TRANSFER_WRITE, TGVK_LAYOUT_SHADER_WRITE_F);
    }
    tgvk_command_buffer_end_and_submit(&p_voxelizer->command_buffer);
...
void tg_voxelizer_end(tg_voxelizer* p_voxelizer, v3i min_corner_index_3d, tg_voxel* p_voxels)
{
    ...
    ((m4*)p_voxelizer->view_projection_ubo.memory.p_mapped_device_memory)[1] = tgm_m4_inverse(tgm_m4_euler(0.0f, TG_DEG2RAD(90.0f), 0.0f));
    ...
    ((m4*)p_voxelizer->view_projection_ubo.memory.p_mapped_device_memory)[1] = tgm_m4_inverse(tgm_m4_euler(TG_DEG2RAD(90.0f), 0.0f, 0.0f));
    ...
    for (u32 i = 0; i < TG_SVO_ATTACHMENTS; i++)
    {
        tgvk_cmd_transition_image_3d_layout(&p_voxelizer->command_buffer, &p_voxelizer->p_image_3ds[i], TGVK_LAYOUT_SHADER_WRITE_F, TGVK_LAYOUT_TRANSFER_READ);
        tgvk_cmd_copy_image_3d_to_buffer(&p_voxelizer->command_buffer, &p_voxelizer->p_image_3ds[i], &p_voxelizer->p_voxel_buffers[i]);
        tgvk_cmd_transition_image_3d_layout(&p_voxelizer->command_buffer, &p_voxelizer->p_image_3ds[i], TGVK_LAYOUT_TRANSFER_READ, TGVK_LAYOUT_SHADER_WRITE_F);
    }
    tgvk_command_buffer_end_and_submit(&p_voxelizer->command_buffer);
...
void tg_voxelizer_destroy(tg_voxelizer* p_voxelizer)
{
    ...
    for (u32 i = 0; i < TG_SVO_ATTACHMENTS; i++)
    {
        ...
        tgvk_image_3d_destroy(&p_voxelizer->p_image_3ds[i]);
    }
