...
tg_storage_buffer_h tg_storage_buffer_create(u64 size, b32 visible)
{
	...
	h_storage_buffer->vulkan_buffer = tgvk_buffer_create(size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, visible ? VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT : VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
void tg_storage_buffer_destroy(tg_storage_buffer_h h_storage_buffer)
{
	...
	tgvk_buffer_destroy(&h_storage_buffer->vulkan_buffer);
}
...
tg_uniform_buffer_h tg_uniform_buffer_create(u64 size)
{
	...
	h_uniform_buffer->vulkan_buffer = tgvk_buffer_create(size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
...
void tg_uniform_buffer_destroy(tg_uniform_buffer_h h_uniform_buffer)
{
	...
	tgvk_buffer_destroy(&h_uniform_buffer->vulkan_buffer);
}
