...
{
	TG_ASSERT(p_camera);
	TG_ASSERT(max_object_count > 0);
	TG_ASSERT(max_object_count <= TGVK_MAX_INSTANCES);
    ...
    const VkDeviceSize instance_data_ssbo_size = max_object_count * sizeof(tg_instance_data_ssbo);
...
void tg_raytracer_create_obj(tg_raytracer* p_raytracer, u32 grid_width, u32 grid_height, u32 grid_depth, f32 center_x, f32 center_y, f32 center_z)
{
    ...
    p_instance_data->r = tgm_m4_rotate_y(TG_DEG2RAD(0.0f));
