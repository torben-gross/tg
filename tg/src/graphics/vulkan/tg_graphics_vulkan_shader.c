...
tg_compute_shader_h tg_graphics_compute_shader_create(u32 input_element_count, tg_shader_input_element* p_input_elements, const char* filename)
{
	...
	compute_shader_h->descriptor_pool = tg_graphics_vulkan_descriptor_pool_create(0, 1, input_element_count, p_descriptor_pool_sizes);
	...
	for (u32 i = 0; i < compute_shader_h->input_element_count; i++)
	{
		...
		case TG_SHADER_INPUT_ELEMENT_TYPE_COMPUTE_BUFFER:
		{
			...
			VkWriteDescriptorSet write_descriptor_set = { 0 };
			{
				...
				write_descriptor_set.descriptorCount = compute_shader_h->p_input_elements[i].array_element_count;
