...
tg_entity_graphics_data_ptr_h tg_entity_graphics_data_ptr_create(tg_entity* p_entity, tg_scene* p_scene)
{
    ...
    *((m4*)entity_graphics_data_ptr_h->uniform_buffer.p_mapped_device_memory) = tgm_m4_identity();
    ...
    for (u32 i = 0; i < p_scene->camera_count; i++)
    {
        ...
        {
            entity_graphics_data_ptr_h->p_entity_scene_infos[i].renderer_h = p_scene->p_deferred_renderers_h[i];
        } break;
        ...
        entity_graphics_data_ptr_h->p_entity_scene_infos[i].descriptor = tg_vulkan_descriptor_create(2, p_descriptor_set_layout_bindings);
        ...
        {
            const VkDescriptorSetLayout p_descriptor_set_layouts[2] = { entity_graphics_data_ptr_h->p_entity_scene_infos[i].descriptor.descriptor_set_layout, p_entity->material_h->descriptor.descriptor_set_layout };
            entity_graphics_data_ptr_h->p_entity_scene_infos[i].pipeline_layout = tg_vulkan_pipeline_layout_create(2, p_descriptor_set_layouts, 0, TG_NULL);
        }
        else
        {
            entity_graphics_data_ptr_h->p_entity_scene_infos[i].pipeline_layout = tg_vulkan_pipeline_layout_create(1, &entity_graphics_data_ptr_h->p_entity_scene_infos[i].descriptor.descriptor_set_layout, 0, TG_NULL);
        }
        ...
        tg_vulkan_graphics_pipeline_create_info vulkan_graphics_pipeline_create_info = { 0 };
        {
            ...
            vulkan_graphics_pipeline_create_info.pipeline_layout = entity_graphics_data_ptr_h->p_entity_scene_infos[i].pipeline_layout;

            switch (p_entity->material_h->material_type)
            {
            case TG_VULKAN_MATERIAL_TYPE_DEFERRED:
            {
                vulkan_graphics_pipeline_create_info.render_pass = p_scene->p_deferred_renderers_h[i]->geometry_pass.render_pass;
            } break;
            case TG_VULKAN_MATERIAL_TYPE_FORWARD:
            {
                vulkan_graphics_pipeline_create_info.render_pass = p_scene->p_forward_renderers_h[i]->shading_pass.render_pass;
            } break;
            default: TG_ASSERT(TG_FALSE);
            }
        }
        entity_graphics_data_ptr_h->p_entity_scene_infos[i].graphics_pipeline = tg_vulkan_graphics_pipeline_create(&vulkan_graphics_pipeline_create_info);
        ...
        VkDescriptorBufferInfo view_projection_descriptor_buffer_info = { 0 };
        {
            ...
            case TG_VULKAN_MATERIAL_TYPE_DEFERRED:
            {
                view_projection_descriptor_buffer_info.buffer = p_scene->p_deferred_renderers_h[i]->camera_h->ubo.buffer;
            } break;
            case TG_VULKAN_MATERIAL_TYPE_FORWARD:
            {
                view_projection_descriptor_buffer_info.buffer = p_scene->p_forward_renderers_h[i]->camera_h->ubo.buffer;
            } break;
            default: TG_ASSERT(TG_FALSE);
            }
        ...
        VkWriteDescriptorSet p_write_descriptor_sets[2] = { 0 };
        {
            ...
            p_write_descriptor_sets[0].dstSet = entity_graphics_data_ptr_h->p_entity_scene_infos[i].descriptor.descriptor_set;
            ...
            p_write_descriptor_sets[1].dstSet = entity_graphics_data_ptr_h->p_entity_scene_infos[i].descriptor.descriptor_set;
        ...
        entity_graphics_data_ptr_h->p_entity_scene_infos[i].command_buffer = tg_vulkan_command_buffer_allocate(graphics_command_pool, VK_COMMAND_BUFFER_LEVEL_SECONDARY);
        ...
        VkCommandBufferInheritanceInfo command_buffer_inheritance_info = { 0 };
        {
            ...
            case TG_VULKAN_MATERIAL_TYPE_DEFERRED:
            {
                command_buffer_inheritance_info.renderPass = p_scene->p_deferred_renderers_h[i]->geometry_pass.render_pass;
            } break;
            case TG_VULKAN_MATERIAL_TYPE_FORWARD:
            {
                command_buffer_inheritance_info.renderPass = p_scene->p_forward_renderers_h[i]->shading_pass.render_pass;
            } break;
            default: TG_ASSERT(TG_FALSE);
            }
            ...
            case TG_VULKAN_MATERIAL_TYPE_DEFERRED:
            {
                command_buffer_inheritance_info.framebuffer = p_scene->p_deferred_renderers_h[i]->geometry_pass.framebuffer;
            } break;
            case TG_VULKAN_MATERIAL_TYPE_FORWARD:
            {
                command_buffer_inheritance_info.framebuffer = p_scene->p_forward_renderers_h[i]->shading_pass.framebuffer;
            } break;
            default: TG_ASSERT(TG_FALSE);
            }
        ...
        tg_vulkan_command_buffer_begin(entity_graphics_data_ptr_h->p_entity_scene_infos[i].command_buffer, VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT, &command_buffer_inheritance_info);
        {
            vkCmdBindPipeline(entity_graphics_data_ptr_h->p_entity_scene_infos[i].command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, entity_graphics_data_ptr_h->p_entity_scene_infos[i].graphics_pipeline);

            const VkDeviceSize vertex_buffer_offset = 0;
            vkCmdBindVertexBuffers(entity_graphics_data_ptr_h->p_entity_scene_infos[i].command_buffer, 0, 1, &p_entity->mesh_h->vbo.buffer, &vertex_buffer_offset);
            if (p_entity->mesh_h->ibo.size != 0)
            {
                vkCmdBindIndexBuffer(entity_graphics_data_ptr_h->p_entity_scene_infos[i].command_buffer, p_entity->mesh_h->ibo.buffer, 0, VK_INDEX_TYPE_UINT16);
            }

            const u32 descriptor_set_count = has_custom_descriptor_set ? 2 : 1;
            VkDescriptorSet p_descriptor_sets[2] = { 0 };
            {
                p_descriptor_sets[0] = entity_graphics_data_ptr_h->p_entity_scene_infos[i].descriptor.descriptor_set;
                if (has_custom_descriptor_set)
                {
                    p_descriptor_sets[1] = p_entity->material_h->descriptor.descriptor_set;
                }
            }
            vkCmdBindDescriptorSets(entity_graphics_data_ptr_h->p_entity_scene_infos[i].command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, entity_graphics_data_ptr_h->p_entity_scene_infos[i].pipeline_layout, 0, descriptor_set_count, p_descriptor_sets, 0, TG_NULL);

            if (p_entity->mesh_h->ibo.size != 0)
            {
                vkCmdDrawIndexed(entity_graphics_data_ptr_h->p_entity_scene_infos[i].command_buffer, (u32)(p_entity->mesh_h->ibo.size / sizeof(u16)), 1, 0, 0, 0); // TODO: u16
            }
            else
            {
                vkCmdDraw(entity_graphics_data_ptr_h->p_entity_scene_infos[i].command_buffer, (u32)(p_entity->mesh_h->vbo.size / sizeof(tg_vertex_3d)), 1, 0, 0);
            }
        }
        VK_CALL(vkEndCommandBuffer(entity_graphics_data_ptr_h->p_entity_scene_infos[i].command_buffer));
    }
...
void tg_entity_graphics_data_ptr_destroy(tg_entity_graphics_data_ptr_h entity_graphics_data_ptr_h)
{
    ...
    for (u32 i = 0; i < entity_graphics_data_ptr_h->scene_info_count; i++)
    {
        tg_vulkan_descriptor_destroy(&entity_graphics_data_ptr_h->p_entity_scene_infos[i].descriptor);
        tg_vulkan_command_buffer_free(graphics_command_pool, entity_graphics_data_ptr_h->p_entity_scene_infos[i].command_buffer);
        tg_vulkan_graphics_pipeline_destroy(entity_graphics_data_ptr_h->p_entity_scene_infos[i].graphics_pipeline);
        tg_vulkan_pipeline_layout_destroy(entity_graphics_data_ptr_h->p_entity_scene_infos[i].pipeline_layout);
    }

    TG_MEMORY_FREE(entity_graphics_data_ptr_h->p_entity_scene_infos);
