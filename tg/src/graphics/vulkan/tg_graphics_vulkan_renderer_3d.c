...
void tgg_renderer_3d_internal_init_geometry_pass(tg_renderer_3d_h renderer_3d_h)
{
    ...
    renderer_3d_h->geometry_pass.view_projection_ubo = tgg_vulkan_buffer_create(sizeof(tg_camera), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
...
void tgg_renderer_3d_internal_init_shading_pass(tg_renderer_3d_h renderer_3d_h, u32 point_light_count, const tg_point_light* p_point_lights)
{
    ...
    renderer_3d_h->shading_pass.graphics_pipeline = tgg_vulkan_graphics_pipeline_create(&vulkan_graphics_pipeline_create_info);
    ...
    renderer_3d_h->shading_pass.exposure.graphics_pipeline = tgg_vulkan_graphics_pipeline_create(&exposure_vulkan_graphics_pipeline_create_info);
    ...
    tgg_vulkan_command_buffer_begin(renderer_3d_h->shading_pass.command_buffer, 0, TG_NULL);
    {
        ...
        vkCmdBindPipeline(renderer_3d_h->shading_pass.command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer_3d_h->shading_pass.graphics_pipeline);
        ...
        vkCmdBindPipeline(renderer_3d_h->shading_pass.command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, renderer_3d_h->shading_pass.exposure.find_exposure_compute_shader.compute_pipeline);
        ...
        vkCmdBindPipeline(renderer_3d_h->shading_pass.command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer_3d_h->shading_pass.exposure.graphics_pipeline);
...
void tgg_renderer_3d_internal_init_present_pass(tg_renderer_3d_h renderer_3d_h)
{
    ...
    renderer_3d_h->present_pass.graphics_pipeline = tgg_vulkan_graphics_pipeline_create(&vulkan_graphics_pipeline_create_info);
    ...
    for (u32 i = 0; i < TG_VULKAN_SURFACE_IMAGE_COUNT; i++)
    {
        ...
        vkCmdBindPipeline(renderer_3d_h->present_pass.command_buffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, renderer_3d_h->present_pass.graphics_pipeline);
...
tg_renderer_3d_h tgg_renderer_3d_create(const tg_camera_h camera_h, u32 point_light_count, const tg_point_light* p_point_lights)
{
    ...
    renderer_3d_h->camera_h = camera_h;
...
void tgg_renderer_3d_begin(tg_renderer_3d_h renderer_3d_h)
{
    ...
    ((tg_camera*)renderer_3d_h->geometry_pass.view_projection_ubo.p_mapped_device_memory)->view = renderer_3d_h->camera_h->view;
    ((tg_camera*)renderer_3d_h->geometry_pass.view_projection_ubo.p_mapped_device_memory)->projection = renderer_3d_h->camera_h->projection;
...
{
    TG_ASSERT(renderer_3d_h && entity_h && entity_h->graphics_data_ptr_h->renderer_3d_h == renderer_3d_h);

    vkCmdExecuteCommands(renderer_3d_h->geometry_pass.command_buffer, 1, &entity_h->graphics_data_ptr_h->command_buffer);
}
