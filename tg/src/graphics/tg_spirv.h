...
typedef enum tg_spirv_bind_type
{
    TG_SPIRV_BIND_TYPE_INVALID,
    TG_SPIRV_BIND_TYPE_BUFFER_BLOCK,
    TG_SPIRV_BIND_TYPE_SAMPLED_IMAGE,
    TG_SPIRV_BIND_TYPE_UNIFORM_BLOCK
} tg_spirv_bind_type;



typedef struct tg_spirv_substructure tg_spirv_substructure;
typedef struct tg_spirv_structure_base
{
    tg_spirv_op               type;
...
{
    tg_spirv_structure_base    base;
    u32                        offset;
} tg_spirv_substructure;

typedef struct tg_spirv_structure
{
    tg_spirv_structure_base    base;
    tg_spirv_bind_type         bind_type;
    u32                        descriptor_set;
    u32                        binding;
} tg_spirv_structure;
...
{
    u8                          version_major_number;
    u8                          version_minor_number;
    u32                         generators_magic_number;
    u32                         bound;
    u32                         schema;

    tg_spirv_execution_model    execution_model;
    char                        p_entry_point[TG_SPIRV_MAX_NAME];

    u32                         structure_capacity;
    u32                         structure_count;
    tg_spirv_structure*         p_structures;
} tg_spirv_layout;



void tg_spirv_create_layout(u32 word_count, const u32* p_words, tg_spirv_layout* p_spirv_layout);
void tg_spirv_destroy_layout(tg_spirv_layout* p_spirv_layout);
